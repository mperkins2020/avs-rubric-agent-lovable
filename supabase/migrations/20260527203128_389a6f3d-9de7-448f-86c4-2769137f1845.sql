...
-- 1) report_feedback: bind email to auth.email()
DROP POLICY IF EXISTS "Authenticated users can insert feedback" ON public.report_feedback;
...
-- 2) scan_usage: bind email to auth.email()
...
DROP POLICY IF EXISTS "No deletes on scan_usage" ON public.scan_usage;
...
-- 3) user_roles: allow users to read their own role; restrict policies to authenticated
DROP POLICY IF EXISTS "No public reads on user_roles" ON public.user_roles;
DROP POLICY IF EXISTS "No deletes on user_roles" ON public.user_roles;
...
-- 4) Fix mutable search_path on get_benchmark_data
...
-- 5) Lock down SECURITY DEFINER trigger helper functions from being callable via the API
