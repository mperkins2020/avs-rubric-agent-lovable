...
const ANALYSIS_VERSION = '2026-08-24-pipeline-v53';
...
6. "Pools and packaging" - Tiers and pools separate exploration from production by segment.
   ...
   **P6 Overage packaging consistency**
   ...
   5. Begin the D6 "rationale" with a compact, machine-readable audit in EXACTLY this format, then the prose — attach the specific field/page citation backing each PASS directly inline in parentheses immediately after the mark, do NOT use a separate evidence line:
      "[D6 audit: P1=P(<field@page-path>)|F P2=P(<exploration_offering@page-path + production_offering@page-path + distinct-construct@page-path>)|F P3=P(<pool1 fields@page-path + pool2 fields@page-path>)|F P4=P(<field@page-path>)|F P5=P(<field@page-path>)|F P6=P(<overage_unit_price@page-path + unit_name@page-path + included_units-or-pool@page-path>)|F | pts=N/6 | gate=<none, or which cap applied> | score=X] "
      THE AUDIT BRACKET IS MANDATORY IN EVERY RESPONSE and must be the FIRST thing in the rationale, before any prose. Emit it even when every subtest is F and even when you could find no citations at all — an all-F bracket is a valid, expected answer for a company with little public evidence. A rationale with no audit bracket is INVALID output. Never drop the bracket, shorten it, or replace it with prose because the marks are mostly failures or because you could not cite a page. Rules: every mark scored P MUST have a non-empty parenthetical citation naming the specific field(s) that passed and the page path of the fact backing each one; for a subtest that passes via a PRICING MODEL CATEGORY AWARENESS override, write "P(auto-seat-based)" — note the hyphenated form, no inner parentheses, since nested parentheses inside the citation will break parsing; marks scored F need NO parenthetical. A subtest marked P with an empty, missing, or "none" citation, one that reuses the same citation across multiple conditions or multiple pools, or cites only assumption-type facts is INVALID — re-mark it F and recompute points, the mapping, and the gates before setting "score".
      EXACT FIELD NAMES REQUIRED: each citation must reference the fields from THAT subtest's Pass conditions — a citation naming anything else does NOT count toward the pass. Specifically:
...
7. "Overages and risk allocation" - Limit behavior is explicit, risk is fairly shared, surprises are prevented.
   ...
   **R6 No-surprise operability**
   ...
   5. Begin the D7 "rationale" with a compact, machine-readable audit in EXACTLY this format, then the prose — attach the specific field/page citation backing each PASS directly inline in parentheses immediately after the mark, do NOT use a separate evidence line:
      "[D7 audit: R1=P(<field@page-path>)|F R2=P(<field@page-path>)|F R3=P(<field@page-path>)|F R4=P(<policy1@page-path + policy2@page-path>)|F R5=P(<invoice/po@page-path + true_up@page-path + overage_behavior@page-path>)|F|NA R6=P(<field@page-path>)|F | pts=N/M | gate=<none, or which cap applied> | score=X] "
      THE AUDIT BRACKET IS MANDATORY IN EVERY RESPONSE and must be the FIRST thing in the rationale, before any prose. Emit it even when every subtest is F and even when you could find no citations at all — an all-F bracket is a valid, expected answer for a company with little public evidence. A rationale with no audit bracket is INVALID output. Never drop the bracket, shorten it, or replace it with prose because the marks are mostly failures or because you could not cite a page. Rules: every mark scored P MUST have a non-empty parenthetical citation naming the specific field(s) that passed and the page path (e.g., "grace_buffer@/pricing") of the fact backing each one; for a subtest that passes via a PRICING MODEL CATEGORY AWARENESS override, write "P(auto-seat-based)" — note the hyphenated form, no inner parentheses, since nested parentheses inside the citation will break parsing; marks scored F or NA need NO parenthetical. A subtest marked P with an empty, missing, or "none" citation, one that duplicates another condition's citation, or cites only assumption-type facts is INVALID — re-mark it F and recompute points, the mapping, and the gates before setting "score".
...
8. "Safety rails and trust surfaces" - Controls prevent surprises, expose usage, and bound risk.
   ...
   **T6 Safe failure behavior and bounded risk**
   ...
   5. Begin the D8 "rationale" with a compact, machine-readable audit in EXACTLY this format, then the prose — attach the specific field/page citation backing each PASS directly inline in parentheses immediately after the mark, do NOT use a separate evidence line:
      "[D8 audit: T1=P(<cap@page-path + admin-config@page-path>)|F T2=P(<field@page-path>)|F T3=P(<field@page-path>)|F T4=P(<field@page-path>)|F T5=P(<path-used-rbac_path-or-soc2_path: field1@page-path + field2@page-path>)|F T6=P(<limit_behavior_docs@page-path + rail@page-path[ + overage_behavior@page-path]>)|F | pts=N/6 | gate=<none, or which cap applied> | score=X] "
      THE AUDIT BRACKET IS MANDATORY IN EVERY RESPONSE and must be the FIRST thing in the rationale, before any prose. Emit it even when every subtest is F and even when you could find no citations at all — an all-F bracket is a valid, expected answer for a company with little public evidence. A rationale with no audit bracket is INVALID output. Never drop the bracket, shorten it, or replace it with prose because the marks are mostly failures or because you could not cite a page. Rules: every mark scored P MUST have a non-empty parenthetical citation naming the specific field(s) that passed and the page path of the fact backing each one; for a subtest that passes via a PRICING MODEL CATEGORY AWARENESS override, write "P(auto-seat-based)" — note the hyphenated form, no inner parentheses, since nested parentheses inside the citation will break parsing; marks scored F need NO parenthetical. A subtest marked P with an empty, missing, or "none" citation, one that reuses the same citation across multiple ANDed conditions within a pass path, or cites only assumption-type facts is INVALID — re-mark it F and recompute points, the mapping, and the gates before setting "score".
      EXACT FIELD NAMES REQUIRED: each citation must reference the fields from THAT subtest's Pass conditions — a citation naming anything else does NOT count toward the pass. Specifically:
...
- Keep rationale to 1-2 sentences max per dimension. Each rationale MUST reference a specific page URL or section (e.g., "Pricing page shows...") — never cite footers, nav items, or copyright notices. The bracketed [D1-D8 audit] block required by each MANDATORY SCORING PROCEDURE section, including its inline per-mark citations, does NOT count toward this sentence limit and must never be omitted or abbreviated to satisfy it.
